...
def main():
    company_list = [] # list of companies in the dataset
    plot_objects = {} 
    for company_file in os.listdir('Data'):
        if company_file == '.DS_Store': # ignore '.DS_Store' in MacOS
            continue
        company = company_file.split('.')[0] # get the name of company, ignoring '.csv'
        company_list.append(company) # concatenate companies into company_list
