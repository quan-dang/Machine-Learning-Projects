...
class PreProcessing:
    ...
    def __init__(self, name):
        name= str(name) # ensure name in string type
        self.get_data(name) # get company data
        self.data['Normalized_Close'] = self.normalized_data_col(self.data) 
        self.data['Quarter'] = self.get_quarter_col(self.data)
        self.num_days = 252 # restrict number of rows in the df, which is roughly the number of trading days in a year
    ...
    def get_data(self, file_name):
        ...
        self.data = self.data.iloc[:, [0, 5]] # select only columns 0 to 5
        self.data = self.data.dropna() # drop record containing NaN
        self.data.Date = pd.to_datetime(self.data.Date)
        self.quarter_names = ['Q' + str(i) for i in range(1, 5)]

    # for each year, normalize the prices to transform yearly series to 0 mean and unit standard deviation
    # also subtract the first day price from all of the data points in that year to force yearly time series starting from 0
    # to avoid any influence from the previous year's prices
